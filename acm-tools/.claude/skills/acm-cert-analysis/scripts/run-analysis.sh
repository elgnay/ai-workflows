        ...
        "Custom-WellKnown")
            ...
            if [ "$SERVER_VERIFICATION_STRATEGY" = "UseAutoDetectedCABundle" ]; then
                if [ "$ROOT_CA_INCLUDED" != "Yes" ]; then
                    ...
                    echo ""
                    echo "Important considerations:"
                    echo "  • ⚠️  Changing intermediate CA or root CA will cause managed clusters to enter unknown state"
                    ...
                    echo "  • You can configure UseSystemTruststore as the KubeAPIServer verification strategy in the global KlusterletConfig to simplify certificate management"
